...
function validateConfig(): void {
  const errors: string[] = [];
  if (!API_TOKEN) errors.push("ADAM_API_TOKEN is not set");
  else if (API_TOKEN.length !== 30) errors.push("ADAM_API_TOKEN must be exactly 30 characters");
  if (!BASE_URL) errors.push("ADAM_BASE_URL is not set");
  else if (!BASE_URL.startsWith("http://") && !BASE_URL.startsWith("https://"))
    errors.push("ADAM_BASE_URL must start with http:// or https://");
  if (errors.length > 0) {
...
// Node 18+ has global fetch; for SSL verification control we use the agent option
// when VERIFY_SSL is false (development only).
const fetchOptions: RequestInit = {};
if (!VERIFY_SSL) {
  // @ts-ignore — Node-specific TLS option
...
async function adamRequest(
  ...
  const parts = [BASE_URL, module, resource, ...params.map(encodeURIComponent)];
...
// ---------------------------------------------------------------------------
// Simple tool executor — builds path from tool definition and calls API
// ---------------------------------------------------------------------------
...
async function handlePupilsFind(args: Record<string, any>): Promise<any> {
  if (!DATAQUERY_PUPILS_SECRET) throw new Error("ADAM_DATAQUERY_PUPILS_SECRET not configured");
  const data = await adamRequest("dataquery", "get", [DATAQUERY_PUPILS_SECRET]);
...
async function handleFamiliesFind(args: Record<string, any>): Promise<any> {
  if (!DATAQUERY_FAMILIES_SECRET) throw new Error("ADAM_DATAQUERY_FAMILIES_SECRET not configured");
  const data = await adamRequest("dataquery", "get", [DATAQUERY_FAMILIES_SECRET]);
  const searchWords = String(args.name).toLowerCase().trim().split(/\s+/);
  const results: any[] = [];
  for (const key of Object.keys(data)) {
    const record = data[key];
...
async function handleStaffFind(args: Record<string, any>): Promise<any> {
  if (!DATAQUERY_STAFF_SECRET) throw new Error("ADAM_DATAQUERY_STAFF_SECRET not configured");
  const data = await adamRequest("dataquery", "get", [DATAQUERY_STAFF_SECRET]);
  const searchWords = String(args.name).toLowerCase().trim().split(/\s+/);
  const results: any[] = [];
  for (const key of Object.keys(data)) {
    const record = data[key];
